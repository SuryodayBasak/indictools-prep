# -*- coding: utf-8 -*-
"""
Created on Thu Aug  6 23:01:25 2015
...
def skew_correction(img):
    ...
    median_area = np.median(dev_areas)
    area_len = len(dev_areas)
    for i in xrange(0,area_len,100):
        if((dev_areas[i]>(-std_dev)) and (dev_areas[i]<=(std_dev))):
